...
class PlayerDTO():
    def __init__(self, summoner_name: str, champion:str, kills: int, assists: int, deaths: int, gold: int, puuid: str, team_color, cs) -> None:
    ...
    def __init__(self,team: list) -> None:
        ...
        for player in self.team:
            self.player_list.append(PlayerDTO(player["summonerName"],player['championName'],player["kills"],\
                player["assists"], player["deaths"], player['goldEarned'], player["puuid"],self.team_color,player["totalMinionsKilled"]+player["neutralMinionsKilled"]) )
...
class MatchDTO(): 
    ...
    def MatchDTOToJSON(self)->dict:
        result = {
            ...
            "thumbnail": {
                "url": f"https://ddragon.leagueoflegends.com/cdn/12.4.1/img/champion/\
                    {self.mainPlayer.champion}.png" #not full proof for certain champions so will go back and make more robust
                }, 
            "author": {
                "name":"Spicy Tuna",
...
# ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def getMatchDTO(self, amount, summoner_name):# eventually will pass in the index so that we can create a list of these objects
        all_game_data = self.SummonerNametoMatchList(amount, summoner_name) #Need to make function that only gets match id list so we can retrieve info 1 json at a time instead of storing a ton of them
        ...
        playerindex = all_game_data[0]["metadata"]["participants"].index(puuid)
        mainPlayer = PlayerDTO(participants[playerindex]["summonerName"],participants[playerindex]['championName'],participants[playerindex]["kills"],\
            ...
            participants[playerindex]["totalMinionsKilled"] + participants[playerindex]["neutralMinionsKilled"])
